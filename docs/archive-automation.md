...
以下内容记录自动化扫描所依赖的游戏界面特征与导航逻辑。修改 `src-tauri/src/navigation/`、`src-tauri/src/task/archive_scan/` 相关代码时，请保持本文档同步更新。
...
每一步跳转后都会检测当前场景；未到达该步目标时，最多执行并检测 3 次。单步重试耗尽，或整条路由执行完毕但未到达最终目标时，会从最后检测到的场景重新规划路由。一次导航最多重新规划 5 次，之后返回导航失败。
