...
pub(in crate::platform) struct PrintWindowState {
    hwnd: HWND,
}
...
impl PrintWindowState {
    ...
    pub(in crate::platform) fn screencap_region(
        &mut self,
        ...
        let full_bitmap = unsafe { CreateCompatibleBitmap(hdc, x1, y1) };
        ...
        let region_bitmap = unsafe { CreateCompatibleBitmap(hdc, width, height) };
