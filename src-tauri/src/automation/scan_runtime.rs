...
use crate::automation::{
    StopToken,
    events::{AppStatus, AutomationOutcome, AutomationRunFinished},
    new_stop_token, request_stop,
...
impl ScanRuntime {
    ...
    fn handle_worker_exit(&self, handle: &AppHandle, result: ScanWorkerExit) {
        ...
        let (public_outcome, scan_error) = match reason {
            FinishReason::Completed => {
                info!("========== 扫描档案库任务执行完毕 ==========");
                (AutomationOutcome::Completed, None)
            }
            FinishReason::Interrupted => {
                info!("扫描档案库任务已被用户停止");
                (AutomationOutcome::Stopped, None)
            }
            FinishReason::Failed(message) => {
                error!("{message}");
                (AutomationOutcome::Failed, Some(message))
            }
        };
